...
// --- [ 1. ส่วนประกอบหน้าบ้าน (Customer View) ] ---
const CustomerPage = ({ mockData, onSearch }) => {
  ...
  return (
    <div className="animate-in fade-in duration-500 w-full max-w-sm space-y-6">
      ...
      <button onClick={() => window.location.href = `https://line.me/ti/p/@singoto-store`} className="w-full bg-[#06C755] text-white py-6 rounded-3xl font-black text-xl flex items-center justify-center gap-3 border-b-4 border-green-800">ติดต่อแอดมิน (LINE)</button>
      ...
      {shipment && (
        <div className="animate-in zoom-in duration-300 bg-white text-black p-8 rounded-[3.5rem] text-center shadow-2xl border-t-8 border-orange-600 flex flex-col items-center mt-6">
          <div className="border border-orange-500 bg-black p-2 px-4 rounded-sm shadow-[0_0_15px_rgba(234,88,12,0.4)] animate-pulse mb-6 text-center text-orange-500 font-mono text-sm tracking-widest uppercase italic">>> MISSION ACCOMPLISHED</div>
...
// --- [ 2. ส่วนประกอบหลังบ้าน (Admin View) ] ---
const AdminPage = ({ onUpload, onExit }) => {
  return (
    <div className="animate-in slide-in-from-bottom duration-500 w-full max-w-sm">
      <div className="p-12 border-4 border-dashed border-orange-500 rounded-[3rem] text-center bg-zinc-900 shadow-2xl" onDrop={(e) => { e.preventDefault(); onUpload(e.dataTransfer.files[0]); }} onDragOver={(e) => e.preventDefault()}>
          ...
          <p className="text-[11px] text-zinc-500 font-bold mb-6 italic">ลากไฟล์ Flash มาวางเพื่ออัปเดตระบบ</p>
          <button onClick={onExit} className="text-[10px] text-zinc-600 underline uppercase tracking-widest hover:text-orange-400">กลับหน้าหลักลูกค้า</button>
      </div>
    </div>
  );
};

// --- [ 3. ส่วนหลักของระบบ (Main System) ] ---
export default function SingtoStore() {
    ...
    reader.onload = (e) => {
      ...
      rows.forEach((row, index) => {
        ...
        if (cols.length > 11) {
          ...
          if (customerPhone && tracking) { newData[customerPhone] = { tracking, status }; count++; }
        }
      });
  ...
  return (
    <div className="min-h-screen bg-black text-white font-sans flex flex-col items-center p-6 relative overflow-hidden border-4 border-orange-600/50 rounded-[3.5rem] shadow-[0_0_60px_rgba(234,88,12,0.3)]">
      ...
      <div className="mt-16 mb-12 text-center relative z-10 select-none cursor-pointer" onDoubleClick={handleAdminToggle}>
      ...
      {isAdminMode ? (
        <AdminPage onUpload={processFile} onExit={() => setIsAdminMode(false)} />
      ) : (
        <CustomerPage mockData={mockData} />
      )}
